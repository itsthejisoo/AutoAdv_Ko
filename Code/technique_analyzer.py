...
def analyze_response_content(response_text, is_initial=False):
    ...
    if any(word in response_lower for word in ["steps", "process", "first", "second", "third", "단계", "절차", "과정", "첫째", "둘째", "셋째", "먼저", "다음으로"]):
        content_type = "procedural"
    elif any(word in response_lower for word in ["example", "instance", "예시", "예를 들", "사례"]):
        content_type = "exemplary"
    elif response_lower.count("?") > 2:
        content_type = "interrogative"
    elif any(word in response_lower for word in ["ethical", "윤리"]) and any(word in response_lower for word in ["concerns", "우려", "고려", "문제"]):
        content_type = "ethical_analysis"
    elif any(word in response_lower for word in ["research", "study", "academic", "연구", "학술", "학문"]):
        content_type = "academic"
    elif any(word in response_lower for word in ["history", "historically", "역사", "역사적"]):
        content_type = "historical"
    elif any(word in response_lower for word in ["alternatives", "instead", "대안", "대신"]):
